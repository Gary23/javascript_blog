...
Base.prototype.center = function(width, height) {
	var Left = (getInner().width - width) / 2 + getScroll().left,
		Top = (getInner().height - height) / 2 + getScroll().top;
	for(var i = 0; i < this.elements.length; i++) {
...
Base.prototype.lock = function() {
	for(var i = 0; i < this.elements.length; i++) {
		this.elements[i].style.width = getInner().width + getScroll().left + 'px'; // 全屏遮罩
...
Base.prototype.resize = function(fn) {
	for(var i = 0; i < this.elements.length; i++) {
		...
		addEvent(window, 'resize', function() {
			fn();
			if(element.offsetLeft > getInner().width + getScroll().left - element.offsetWidth) { // 浏览器窗口由大缩小时，不让调用该方法的元素超出窗口范围
